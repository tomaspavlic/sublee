...
namespace OpenSubtitles.Model
{
    public class XmlRpcClient
    {
        ...
        public T Invoke<T>(string methodName, params object[] parameters)
        {
            ...
            HttpResponseMessage xx = _httpClient.PostAsync(_serviceUrl, content).Result;
            if (xx.IsSuccessStatusCode)
            {
                string responseString = xx.Content.ReadAsStringAsync().Result;
                T o = DeserializeResponse<T>(responseString);
                return o;
            }
            else
            {
                throw new HttpRequestException(xx.Content.ReadAsStringAsync().Result);
            }
        }
