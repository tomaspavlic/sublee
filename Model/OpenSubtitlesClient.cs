...
namespace OpenSubtitles.Model
{
    public class OpenSubtitlesClient
    {
        ...
        public OpenSubtitlesClient(string OSDBApiUrl)
        {
            _rcpClient = new XmlRpcClient(OSDBApiUrl);
        }

        public void LogIn(string language, string agent)
        {
            LogIn logIn = _rcpClient.Invoke<LogIn>("LogIn", string.Empty, string.Empty, language, agent);
            _token = logIn.Token;
        }
